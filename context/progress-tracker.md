...
- Created `apps/api/Dockerfile` using Python 3.11-slim with system dependencies for OpenCV and InsightFace, and updated `docker-compose.yml` to include the API service container with volume mounting for enrollment media.
- Updated `README.md` with comprehensive developer startup instructions, detailing prerequisites, environment configuration, full Docker Compose option, and local hot-reloading dev server setup for API and Web.
