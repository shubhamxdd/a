...
export function TeacherClassroom({ classroom, onActiveSessionChange }: { classroom: Classroom; onActiveSessionChange?: (session: AttendanceSession | null) => void }) {
  ...
  const [recognitionInterval, setRecognitionInterval] = useState(15)
  ...
  const intervalExceedsWindow = recognitionInterval > windowMinutes * 60
  const formatInterval = (seconds: number) => seconds >= 60 ? `${seconds / 60} minute${seconds >= 120 ? 's' : ''}` : `${seconds} seconds`
  const start = async () => { try { setMessage(''); const roomCameras = await api.listTeacherRoomCameras(roomCode); setCameras(roomCameras); setActiveSession(await api.startSession(classroom.id, { title: sessionTitle, room_code: roomCode, qualification_window_minutes: windowMinutes, recognition_interval_seconds: recognitionInterval, grace_period_minutes: graceMinutes })); await load() } catch (e) { setMessage(e instanceof Error ? e.message : 'Unable to start session.') } }
